...
with tab3:
    ...
    if not filtered_df.empty:
        ...
        fig_scatter = px.scatter(
            ...
            x="Discount",
            y="Profit",
            title="1. Discount vs Profit",
            opacity=0.5
        )
        fig_scatter.update_traces(marker=dict(color="darkgreen"))

        if len(filtered_df) > 1:
            z = np.polyfit(filtered_df["Discount"], filtered_df["Profit"], 1)
            p = np.poly1d(z)
            x_line = np.linspace(filtered_df["Discount"].min(), filtered_df["Discount"].max(), 100)
            y_line = p(x_line)
        ...
        st.markdown("""
**Insight:** The scatter plot shows a negative relationship between discount and profit.  
As discount levels rise, profitability tends to decline, suggesting that excessive discounting may damage margins.
""")
        ...
        fig_line = px.line(
            ...
            x="Discount",
            y="Profit",
            title="2. Average Profit by Discount Level",
            markers=True
        )
        ...
        st.markdown("""
**Insight:** Average profit generally decreases as discount levels increase.  
This suggests that although discounts may support sales volume, they often reduce average profitability per transaction.
""")
        ...
        st.markdown("""
**Insight:** Orders with no or low discount tend to maintain stronger profitability, while medium and high discount groups show weaker results.  
This highlights the trade-off between promotional pricing and margin protection.
""")
    else:
        st.write("No data available for the selected filters.")
...
with tab4:
    ...
    if not filtered_df.empty:
        ...
        # 关键修复：保留完整产品名用于 hover，同时做短名称用于坐标轴显示
        ...
        fig_products.update_layout(
            ...
            xaxis_title="Sales",
            yaxis_title="Product",
            height=max(500, top_n * 45),   # 产品越多，图越高
        ...
        st.markdown("""
**Insight:** Sales are highly concentrated among a small number of products, indicating reliance on key revenue drivers.  
Strategic focus on these products could improve performance, but overdependence may also create concentration risk.
""")
